...
export function AddExpenseModal({ isOpen, onClose }: AddExpenseModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-2xl max-h-[90vh] overflow-hidden rounded-t-3xl sm:rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 animate-slide-up">
        {/* Success Overlay */}
        {showSuccess && (
          <div className="absolute inset-0 bg-slate-900/95 backdrop-blur-md z-10 flex items-center justify-center">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">Expense Created!</h3>
              <p className="text-slate-400">Your expense has been recorded successfully</p>
            </div>
          </div>
        )}

        {/* Header */}
        <div className="sticky top-0 z-10 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-b border-white/10 px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold text-white">Add Expense</h2>
              <p className="text-slate-400 text-sm">Record a new expense transaction</p>
            </div>
            <button
              onClick={onClose}
        ...
        {/* Form */}
        <form onSubmit={handleSubmit} className="overflow-y-auto max-h-[calc(90vh-80px)] px-6 py-6">
          <div className="space-y-5">
            ...
            <div>
              ...
              <select
                ...
                required
                className="w-full px-4 py-3.5 bg-white/5 border border-white/10 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent/50 transition-all"
              >
            ...
            <div>
              ...
              <input
                ...
                placeholder="0.00"
                required
                className="w-full px-4 py-3.5 bg-white/5 border border-white/10 rounded-xl text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent/50 transition-all"
              />
            </div>
              ...
              <div>
                ...
                >
                  ...
                  {availableSubCategories.map((subCategory) => (
                    <option key={subCategory} value={subCategory} className="bg-slate-800">
                      {subCategory}
                    </option>
                  ))}
                </select>
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-4 py-3.5 bg-white/5 border border-white/10 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent/50 transition-all"
              >
                <option value="" className="bg-slate-800">Select payment account</option>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-3.5 bg-white/5 border border-white/10 rounded-xl text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent/50 transition-all resize-none"
              />
            </div>
              ...
              <Card className="p-4 bg-accent/10 border-accent/30 rounded-xl">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="text-white font-bold">Br {parseFloat(formData.expenseAmount).toFixed(2)}</span>
                  </div>
                  {formData.paidNow && (
                    <>
                      <div className="flex justify-between">
                        <span className="text-slate-400">Paid Now:</span>
                        <span className="text-green-400 font-bold">Br {parseFloat(formData.paidNow).toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between border-t border-white/10 pt-2">
                        <span className="text-slate-400">Remaining:</span>
                        <span className={`font-bold ${parseFloat(remainingBalance) > 0 ? 'text-yellow-400' : 'text-green-400'}`}>
                          Br {remainingBalance}
                        </span>
                      </div>
                    </>
            ...
            {/* Submit Button */}
            <div className="sticky bottom-0 bg-gradient-to-t from-slate-900 via-slate-900 to-transparent pt-6 -mx-6 px-6 pb-2">
              <button
                type="submit"
